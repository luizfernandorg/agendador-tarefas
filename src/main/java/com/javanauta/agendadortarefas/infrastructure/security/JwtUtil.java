...
public class JwtUtil {
    ...
    private SecretKey getSecretyKey() {
    ...
    public Claims extractClaims(String token) {
        return Jwts.parser()
                .verifyWith(getSecretyKey()) // Define a chave secreta para validar a assinatura do token
                .build()
                .parseSignedClaims(token) // Analisa o token JWT e obtém as claims
                .getPayload(); // Retorna o payload das claims
    }
